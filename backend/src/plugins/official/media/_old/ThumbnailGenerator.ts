...
export default class ThumbnailGenerator {
   ...
   *        Actually, this has also been copied from a previous version of Apollo now too ^^
   */

  async generateThumbnail(file: LocalFile): Promise<{ mime: string, data: Buffer } | null> {
    const mimeType = await this.fileTypeUtils.getMimeType(file.getAbsolutePathOnHost());
    ...
    const sharpInstance = sharp(await file.read(), {
      failOnError: false,
      sequentialRead: true,
      density: 300,
    })
  ...
  private async generateVideoThumbnail(file: LocalFile, sampleSize: number = 3, width = 500): Promise<{
    img: Sharp
  }> {
    if (sampleSize <= 0) {
