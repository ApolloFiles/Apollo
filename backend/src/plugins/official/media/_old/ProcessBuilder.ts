...
export default class ProcessBuilder {
  ...
  run(): IChildProcess {
    ...
    const cwd = this.cwd ?? Path.join(container.resolve(ApolloDirectoryProvider).getAppTemporaryDirectory(), `process_${processUniqueId}`);

    Fs.mkdirSync(cwd, { recursive: true });

    const process = container.resolve(ProcessManager).create(processUniqueId, this.command, this.args, {
      cwd,
      stdio: this.stdio,
      env: this.env,
  ...
  runPromised(): Promise<{
    ...
    code: ChildProcessExitCode,
    signal: ChildProcessExitSignal,
    err?: Error
  }> {
