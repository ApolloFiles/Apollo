...
export default class TemporaryDirectory {
  ...
  static create(uid: string): TemporaryDirectory {
    const tmpDir = Path.join(container.resolve(ApolloDirectoryProvider).getAppTemporaryDirectory(), 'watch', 'sessions', uid); // TODO: Make this whole path generation more portable/reusable
