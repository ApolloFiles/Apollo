...
export class TheTvDatabaseApiClient {
  ...
  async fetchSeriesById(id: number): Promise<TvDbSeries | null> {
    ...
    if (seriesRes.statusCode === 404) {
      return null;
    }
    if (seriesRes.statusCode !== 200) {
      throw new Error('Failed to fetch series from TheTvDatabase API (Status ' + seriesRes.statusCode + '): ' + seriesRes.body.toString());
    }
  ...
  async fetchMovieById(id: number): Promise<TvDbMovie | null> {
    ...
    if (movieRes.statusCode !== 200) {
      throw new Error('Failed to fetch movie from TheTvDatabase API (Status ' + movieRes.statusCode + '): ' + movieRes.body.toString());
    }
  ...
  private parseAsSimpleEntry(value: any): TvDbEntry {
    ...
    if (yearStr != null && !StringUtils.default.isNumeric(yearStr)) {
  ...
  private async login(): Promise<void> {
    const loginRes = await this.httpClient.post(`${this.BASE_URL}/login`, {
      headers: {
        Accept: 'application/json',
