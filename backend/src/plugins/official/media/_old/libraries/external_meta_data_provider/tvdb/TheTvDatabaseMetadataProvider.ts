...
import { TheTvDatabaseApiClient, type TvDbMovie, type TvDbSeries } from './TheTvDatabaseApiClient.js';
...
export default class TheTvDatabaseMetadataProvider extends MetadataProvider {
  ...
  constructor(
    ...
    this.theTvDatabaseClient = new TheTvDatabaseApiClient(apiKey, httpClient);
  }
  ...
  private convertToTitleMetaData(response: TvDbSeries | TvDbMovie | null): TitleMetaData | null {
    ...
    return {
      ...
      fetchPosterImage: async () => {
        ...
        const coverResponse = await this.httpClient.get(response.image);
        ...
        throw new Error(`Failed to download poster image from '${response.image}': ${coverResponse.statusCode} ${coverResponse.body.toString('utf-8')}`);
      },
