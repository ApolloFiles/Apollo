...
import MyAnimeListApiClient, { type MyAnimeListAnime } from './MyAnimeListApiClient.js';
...
export default class MyAnimeListMetadataProvider extends MetadataProvider {
  ...
  constructor(
    ...
    this.myAnimeListClient = new MyAnimeListApiClient(clientId, httpClient);
  }
  ...
  async fetchByMetaProvider(metaProvider: MetaProvider): Promise<TitleMetaData | null> {
    ...
    if (!StringUtils.default.isNumeric(metaProvider.mediaId)) {
  ...
  private convertToTitleMetaData(response: MyAnimeListAnime | null): TitleMetaData | null {
    ...
    return {
      ...
      fetchPosterImage: async () => {
        ...
        const coverResponse = await this.httpClient.get(response.coverImageUrl);
        if (coverResponse.statusCode === 404) {
          return null;
        }
        if (coverResponse.statusCode === 200) {
          return coverResponse.body;
        }
        throw new Error(`Failed to download poster image from '${response.coverImageUrl}': ${coverResponse.statusCode} ${coverResponse.body.toString('utf-8')}`);
      },
