...
describe('#prettifyFileSize', () => {
  ...
  test('With negative bytes', () => {
    expect(() => Utils.prettifyFileSize(-1)).toThrowError();
    expect(() => Utils.prettifyFileSize(Number.MIN_SAFE_INTEGER)).toThrowError();

    expect(() => Utils.prettifyFileSize(-1, true)).toThrowError();
    expect(() => Utils.prettifyFileSize(Number.MIN_SAFE_INTEGER, true)).toThrowError();
  });

  test('With invalid values for bytes', () => {
    expect(() => Utils.prettifyFileSize(Number.NaN)).toThrowError();
    expect(() => Utils.prettifyFileSize(undefined as any)).toThrowError();
    expect(() => Utils.prettifyFileSize(null as any)).toThrowError();
    expect(() => Utils.prettifyFileSize('10' as any)).toThrowError();

    expect(() => Utils.prettifyFileSize(Number.NaN, true)).toThrowError();
    expect(() => Utils.prettifyFileSize(undefined as any, true)).toThrowError();
    expect(() => Utils.prettifyFileSize(null as any, true)).toThrowError();
    expect(() => Utils.prettifyFileSize('10' as any, true)).toThrowError();
  });
});
